...
| `req-marker` 包含展开/隐藏区域，角标位置偏移 | 只包裹可见的核心UI组件 |

**🎯 角标位置精确性规则（重要！）：**

1. **紧邻原则**：角标必须出现在目标组件的**紧邻右上角**，距离不超过20px
2. **可见性原则**：`req-marker` 只包裹**始终可见**的核心UI组件
3. **排除原则**：以下内容**禁止**包含在 `req-marker` 内：
   - 展开/折叠区域（如高级筛选、详情面板）
   - 隐藏区域（如 `hidden` 类控制的DOM）
   - 弹窗/抽屉/模态框（应独立标注）
   - 分页、提示文字等辅助元素

```html
<!-- ❌ 错误：req-marker 包含展开区域，角标位置偏移 -->
<div class="req-marker" id="req-marker-2">
    <span class="req-badge" data-req="2">2</span>
    <div class="filter-bar"><!-- 基础筛选 --></div>  <!-- 角标离这里很远 -->
    <div class="advanced-filter hidden"><!-- 高级筛选 --></div>  <!-- 导致容器过大 -->
</div>

<!-- ✅ 正确：req-marker 只包裹基础筛选栏 -->
<div class="mb-6">
    <div class="req-marker relative" id="req-marker-2">  <!-- 精确包裹 -->
        <span class="req-badge" data-req="2">2</span>
        <div class="filter-bar"><!-- 基础筛选 --></div>  <!-- 角标在右上角 -->
    </div>
    <!-- 展开区域独立在外 -->
    <div class="advanced-filter hidden"><!-- 高级筛选 --></div>
</div>
...
- [ ] **位置紧邻性**：角标是否在目标组件的**紧邻右上角**（距离≤20px）？是否远离目标组件？
- [ ] **可见性检查**：`req-marker` 是否只包含**始终可见**的核心UI组件？是否错误包含了展开/隐藏区域？
- [ ] **排除项检查**：以下内容是否**排除**在 `req-marker` 外？
  - 展开/折叠区域（高级筛选、详情面板等）
  - 隐藏区域（`hidden` 类控制的DOM）
  - 弹窗/抽屉/模态框（应独立标注）
  - 分页、提示文字等辅助元素
